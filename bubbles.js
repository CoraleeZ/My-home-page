...
function drawName(name, letterColors) {
    ...
    for (var j = 0; j < g.length; j++) {
        ...
        g[j].curPos.y = (canvasHeight / 2 - 35) + g[j].curPos.y;
        g[j].originalPos.x = 20 + g[j].originalPos.x;
        //(canvasWidth / 2 - offset / 2) + g[j].originalPos.x;
        g[j].originalPos.y = (canvasHeight / 2 - 45) + g[j].originalPos.y;
    }
...
setTimeout(updateCanvasDimensions, 20);

message = "Coralee Q's Diary!";

drawName(message, [red, blue, purple, orange, green]);
bounceBubbles();